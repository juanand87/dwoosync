<?php
/**
 * Ejemplo de cómo incrementar sync_count desde el plugin de WordPress
 * Este código debe integrarse en el plugin de WordPress
 */
...
class DwooSyncCountManager {
    ...
    public function __construct($license_key, $domain) {
        $this->api_url = 'https://tu-dominio.com/api_discogs/api/increment_sync_count.php';
    ...
    public function incrementSyncCount() {
        ...
        if ($result['success']) {
            error_log('Sync count incrementado: ' . $result['data']['sync_count']);
            return $result['data'];
        } else {
            error_log('Error incrementando sync_count: ' . $result['message']);
            return false;
        }
    }
}

// Ejemplo de uso en el plugin de WordPress
function handle_import_button_click() {
    ...
    if ($result) {
        ...
        add_action('admin_notices', function() use ($result) {
            ...
            echo 'Sincronización exitosa. Total de sincronizaciones: ' . $result['sync_count'];
            echo '</p></div>';
        });
    } else {
