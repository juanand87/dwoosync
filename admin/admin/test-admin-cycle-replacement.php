<?php
/**
 * Script para probar el reemplazo de ciclos pendientes desde el admin
 */
...
echo "<h1>Prueba de Reemplazo de Ciclos desde Admin</h1>";

// Habilitar reporte de errores
error_reporting(E_ALL);
ini_set('display_errors', 1);

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['test_admin_replacement'])) {
    $subscriber_id = $_POST['subscriber_id'] ?? '';
    
    if (!empty($subscriber_id)) {
        echo "<h2>Probando reemplazo con suscriptor ID: $subscriber_id</h2>";
        
        // Obtener todos los ciclos del suscriptor ANTES
        $cycles_before = $db->query("
        ...
        // Simular la lógica del admin
        echo "<h3>Simulando creación de ciclo desde admin...</h3>";
        ...
        try {
            ...
            // 3. Crear nuevo ciclo pendiente (simulando la lógica del admin)
            echo "<p>🔄 Creando nuevo ciclo pendiente...</p>";
            
            $cycle_start = date('Y-m-d');
            $cycle_end = date('Y-m-d', strtotime('+30 days'));
            $due_date = date('Y-m-d', strtotime('+7 days'));
            $invoice_number = 'ADMIN-' . date('Y') . '-' . str_pad($subscriber_id, 6, '0', STR_PAD_LEFT) . '-' . str_pad(time(), 4, '0', STR_PAD_LEFT);
            
            $new_cycle_id = $db->insert('billing_cycles', [
                'subscriber_id' => $subscriber_id,
                'plan_type' => 'premium',
                'license_key' => 'ADMIN-' . $subscriber_id . '-' . time(),
            ...
            echo "<h3>Ciclos DESPUÉS del test:</h3>";
            ...
            if ($pending_count == 1) {
                ...
                echo "<h3>✅ ÉXITO</h3>";
                echo "<p>La lógica de reemplazo funciona correctamente desde el admin. Solo hay $pending_count ciclo(s) pendiente(s).</p>";
                echo "</div>";
            } else {
...
if (empty($subscribers)) {
    ...
    foreach ($subscribers as $subscriber) {
        ...
        echo "<button type='submit' name='test_admin_replacement' style='background: #28a745; color: white; padding: 5px 10px; border: none; border-radius: 4px; cursor: pointer;'>Probar Admin</button>";
        echo "</form>";
...
echo "<hr>";
echo "<p><a href='licenses.php' style='color: #007bff;'>📊 Volver a Gestión de Licencias</a></p>";
?>
