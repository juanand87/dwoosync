...
<body>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (container) {
                paypal.Buttons({
                    ...
                    // Pago aprobado - PREMIUM
                    onApprove: function(data, actions) {
                        // Mostrar procesamiento y redirigir a exito en popup
                        showPaymentProcessing();
                        const redirectUrl = `${baseURL}exito.php?billing_cycle_id=${encodeURIComponent(billingCycleId)}&sub=${encodeURIComponent(data.subscriptionID)}&popup=1`;
                        
                        setTimeout(() => {
                            // Intentar abrir popup
                            paymentPopup = window.open(redirectUrl, 'payment_success', 'width=600,height=500,scrollbars=yes,resizable=yes');
                            
                            // Verificar si el popup fue bloqueado
                            if (!paymentPopup || paymentPopup.closed || typeof paymentPopup.closed == 'undefined') {
                                // Popup bloqueado, usar redirección normal como fallback
                                updateProcessingMessage('<?php echo t("Los popups están bloqueados. Redirigiendo...", "Popups are blocked. Redirecting..."); ?>');
                                setTimeout(() => {
                                    hidePaymentProcessing();
                                    const fallbackUrl = redirectUrl.replace('&popup=1', '');
                                    window.location.href = fallbackUrl;
                                }, 1500);
                                return;
                            }
                            
                            // Popup funcionó, actualizar mensaje
                            updateProcessingMessage('<?php echo t("Completa el pago en la ventana de PayPal...", "Complete the payment in the PayPal window..."); ?>');
                            
                            // Popup funcionó, verificar si fue cerrado manualmente
                            const checkClosed = setInterval(() => {
                                if (paymentPopup && paymentPopup.closed) {
                                    clearInterval(checkClosed);
                                    hidePaymentProcessing();
                                    // Redirigir a página de cancelación cuando se cierra manualmente
                                    setTimeout(() => {
                                        const cancelUrl = `${baseURL}error_pago.php?error=cancelled&plan=premium&billing_cycle_id=${encodeURIComponent(billingCycleId)}&lang=${currentLang}`;
                    ...
                    onCancel: function(data) {
                        // Manejar cancelación - PREMIUM PLAN
                        stopPaymentPolling();
                        hidePaymentProcessing();
                        const cancelUrl = `${baseURL}error_pago.php?error=cancelled&plan=premium&billing_cycle_id=${encodeURIComponent(billingCycleId)}&lang=${currentLang}`;
                        window.location.href = cancelUrl;
                    ...
                    onError: function(err) {
                        ...
                        const errorUrl = `${baseURL}error_pago.php?error=${errorType}&plan=premium&billing_cycle_id=${encodeURIComponent(billingCycleId)}&lang=${currentLang}`;
            ...
            if (enterpriseContainer) {
                paypal.Buttons({
                        ...
                        setTimeout(() => {
                            ...
                            const checkClosed = setInterval(() => {
                                if (paymentPopup && paymentPopup.closed) {
                                    ...
                                    // Redirigir a página de cancelación cuando se cierra manualmente
                                    setTimeout(() => {
                                        const cancelUrl = `${baseURL}error_pago.php?error=cancelled&plan=enterprise&billing_cycle_id=${encodeURIComponent(billingCycleId)}&lang=${currentLang}`;
                                        window.location.href = cancelUrl;
                                    }, 1000);
                                }
                            }, 1000);
                    ...
                    // Pago cancelado por el usuario
                    onCancel: function(data) {
                        // Manejar cancelación - ENTERPRISE PLAN
                        stopPaymentPolling();
                        hidePaymentProcessing();
                        const cancelUrl = `${baseURL}error_pago.php?error=cancelled&plan=enterprise&billing_cycle_id=${encodeURIComponent(billingCycleId)}&lang=${currentLang}`;
                        window.location.href = cancelUrl;
                    ...
                    onError: function(err) {
                        ...
                        const errorUrl = `${baseURL}error_pago.php?error=${errorType}&plan=enterprise&billing_cycle_id=${encodeURIComponent(billingCycleId)}&lang=${currentLang}`;
                        window.location.href = errorUrl;
                    }
    ...
    <script>
        ...
        // Sistema de popup PayPal y polling
        ...
        function openPayPalPopup(url) {
            ...
            paymentPopup = window.open(url, 'paypal_payment', 'width=500,height=600,scrollbars=yes,resizable=yes');
            ...
            const checkClosed = setInterval(() => {
                if (paymentPopup.closed) {
                    ...
                    stopPaymentPolling();
                    hidePaymentProcessing();
                    // Redirigir a página de error cuando se cierra manualmente
                    setTimeout(() => {
                        const plan = '<?php echo $planType; ?>';
                        const cancelUrl = `${baseURL}error_pago.php?error=cancelled&plan=${plan}&billing_cycle_id=${encodeURIComponent(billingCycleId)}&lang=${currentLang}`;
                        window.location.href = cancelUrl;
                    }, 1000);
                }
            }, 1000);
        ...
        function showPaymentProcessing() {
            // Crear overlay de procesamiento si no existe
                ...
                overlay.innerHTML = `
                    ...
                    ">
                        ...
                        ">
                            ...
                            <h3 style="margin: 0 0 15px 0; color: #333;"><?php echo t('Procesando Pago', 'Processing Payment'); ?></h3>
                            <p id="processing-message" style="margin: 0 0 20px 0; color: #666;"><?php echo t('Abriendo ventana de confirmación...', 'Opening confirmation window...'); ?></p>
                            <div style="
                                width: 40px;
                                height: 40px;
                            ...
                            <button onclick="cancelPayment()" style="
                                ...
                                border-radius: 5px;
                                cursor: pointer;
                            "><?php echo t('Cancelar', 'Cancel'); ?></button>
                        </div>
                    </div>
                    <style>
                        @keyframes spin {
        ...
        function startPaymentPolling() {
            pollingInterval = setInterval(() => {
                fetch(`check_payment_status.php?billing_cycle_id=${billingCycleId}`)
                    .then(response => response.json())
                    .then(data => {
                        if (data.status === 'paid') {
                            ...
                            hidePaymentProcessing();
                            window.location.href = `error_pago.php?error=${data.status}&billing_cycle_id=${billingCycleId}`;
                        }
                    })
                    .catch(error => {
        ...
        function showPaymentSuccess() {
            window.location.href = `exito.php?billing_cycle_id=${billingCycleId}&success=1&lang=<?php echo $currentLang; ?>`;
        }
        ...
        window.addEventListener('message', function(event) {
            ...
            if (event.data.type === 'payment_success') {
                ...
                hidePaymentProcessing();
                window.location.href = `error_pago.php?error=failed&billing_cycle_id=${billingCycleId}`;
            }
        });
    </script>
</body>
