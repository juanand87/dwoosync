...
// Si no hay datos de signup pero hay sesión activa, cargar desde BD
if (empty($signupData) && isset($_SESSION['subscriber_id'])) {
    try {
        $pdo = getDatabase();
        $stmt = $pdo->prepare("SELECT first_name, last_name, email, domain FROM subscribers WHERE id = ?");
        $stmt->execute([$_SESSION['subscriber_id']]);
        $userData = $stmt->fetch(PDO::FETCH_ASSOC);
        
        if ($userData) {
            $signupData = $userData;
            // Actualizar las variables de sesión para futuras consultas
            $_SESSION['user_name'] = $userData['first_name'] . ' ' . $userData['last_name'];
            $_SESSION['user_email'] = $userData['email'];
            $_SESSION['user_domain'] = $userData['domain'];
        }
    } catch (Exception $e) {
        error_log('Error loading user data in checkout: ' . $e->getMessage());
    }
}
...
<body>
    ...
    <main style="padding-top: 100px; min-height: 100vh; background: #ffffff;">
        <div class="container">
            <div class="page-header">
                ...
                <p><?php echo t('Revisa tu plan y procede al pago', 'Review your plan and proceed to payment'); ?></p>
            </div>
            ...
            <!-- Selector de Planes -->
                <div id="planSelector" style="display: none; background: #f8fafc; border-radius: 12px; padding: 1.5rem; margin-bottom: 2rem; border: 2px solid #e5e7eb;">
                    <h4 style="margin: 0 0 1rem 0; color: #1f2937;">
                        <i class="fas fa-list"></i> <?php echo t('Selecciona tu Plan', 'Select your Plan'); ?>
                    </h4>
                        ...
                        <?php foreach ($plans as $plan): ?>
                            <div class="plan-option" 
                                ...
                                <div style="font-size: 1.2rem; font-weight: bold; color: #10b981; margin-bottom: 0.5rem;">
                                 $<?php echo $plan['price']; ?>/<?php echo t('mes', 'month'); ?>
                                </div>
                                ...
                                <?php if ($plan['id'] === $planType): ?>
                                    <div class="selected-indicator" style="text-align: center; margin-top: 0.5rem; color: #3b82f6; font-weight: bold;">
                                        <i class="fas fa-check-circle"></i> <?php echo t('Seleccionado', 'Selected'); ?>
                                    </div>
                                <?php endif; ?>
                            </div>
                        <?php endforeach; ?>
    ...
    <script>
        ...
        function updatePlanDisplay(element) {
            ...
            document.getElementById('plan-price').textContent = '$' + planPrice + '/<?php echo t('mes', 'month'); ?>';
            ...
            if (buttonText) {
                ...
                const createAccountText = isEnglish ? 'Create Account' : 'Crear Cuenta';
                const proceedPaymentText = isEnglish ? 'Proceed to Payment' : 'Proceder al Pago';
                buttonText.textContent = planId === 'free' ? createAccountText : proceedPaymentText;
            }
        ...
        function updatePlanSelection(element) {
            ...
            planOptions.forEach(option => {
                ...
                if (option.dataset.planId === planId) {
                    ...
                    const isEnglish = <?php echo $isEnglish ? 'true' : 'false'; ?>;
                    const selectedText = isEnglish ? 'Selected' : 'Seleccionado';
                    indicator.innerHTML = '<i class="fas fa-check-circle"></i> ' + selectedText;
